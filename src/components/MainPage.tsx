...
import { claimRewardTx, getUserState, unlockPnft } from "@/transaction/cli/txScripts";
...
export default function MainPage() {
  ...
  const handleSelect = (nft: Nft) => {
    ...
    const index = updatedSelection.findIndex(s => s.mint === nft.mint);

    // Calculating if the NFT is locked based on lockTime and the plan
    const isLocked = (new Date().getTime() / 1000 - nft.lockTime) / DAY <
      PLANS[`PLAN_${nft.rate / 100_000_000 - 1}` as keyof typeof PLANS];

    if (!nft.staked && isLocked) {
      toast.error("Locked!");
      return;
    }
    ...
    if (!isLocked) {
      if (index !== -1 || updatedSelection.length === MAX_SELECTABLE) {
        updatedSelection.splice(index, 1);
      } else {
        updatedSelection.push(nft);
      }
    }
  ...
  const handleUnStake = async () => {
    setUnstaking(true);
    try {
      await unlockPnft(wallet, selected.map(item => new PublicKey(item.mint)));
  ...
  const fetchData = async () => {
    if (publicKey) {
      try {
        const uData = await getUserState(publicKey);
        if (uData) {
          let sum = 0;
          for (let i = 0; i < uData.itemCount.toNumber(); i++) {
            const rate = uData.items[i].rate.toNumber();
            const range = (new Date().getTime() / 1000 - uData.items[i].rewardTime.toNumber()) / DAY;
            sum += rate * range;
          }
                ...
                {nfts.length > 0 ? (
                  <div className="pt-10 relative z-30">
                    <div className="relative z-30">
                      <div className="w-full border-b-2 border-white/10">
                        <div className="translate-y-0.5">
                          <button
                            className={`px-3 md:px-10 py-2 font-medium border-2 text-white border-none w-1/2 md:w-auto text-sm md:text-[16px] ${tab === "unstaked" ? "tab-active" : ""}`}
                            onClick={() => setTab("unstaked")}
                          >
                            Unstaked ({unStakedNfts.length})
                          </button>
                          <button
                            className={`px-3 md:px-10 py-2 font-medium border-2 text-white border-none w-1/2 md:w-auto text-sm md:text-[16px] ${tab === "staked" ? "tab-active" : ""}`}
                    ...
                    {selected.length > 0 && (
                      <div className="mt-10 flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-2 md:mt-0">
                          ...
                          <button
                            className={`px-5 border font-medium rounded-lg bg-[#e14735] hover:bg-[#e14700] duration-200 disabled:opacity-70 border-white/20 disabled:pointer-events-none text-white md:w-[200px] grid place-content-center h-8 md:h-10 text-xs md:text-sm ${tab === "staked" ? "" : "hidden"}`}
                          ...
                          <button
                            className={`px-5 border font-medium rounded-lg bg-[#e14735] hover:bg-[#e14700] duration-200 disabled:opacity-70 border-white/20 disabled:pointer-events-none text-white md:w-[200px] grid place-content-center h-8 md:h-10 text-xs md:text-sm ${tab === "unstaked" ? "" : "hidden"}`}
                            disabled={selected.length === 0}
                            onClick={openStake}
                          >
                            Stake
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-4 xl:grid-cols-5 gap-4 mt-4">
                      {(tab === "staked" ? stakedNfts : unStakedNfts).map((nft, index) => (
                        <NftCard
                          key={`${index}-${nft.mint}`}
                          nft={nft}
                          refetch={fetchNfts}
